...
class TestAgentBlocklistHelpers:
    ...
    def test_contains_fingerprint_passes_email_in_code_span(self):
        """An identity quoted in an inline code span is prose, not attribution.

        Refs: #1521 -- describing a bot-identity bug must not block the change.
        """
        blocklist = {
            "trailers": [],
            "names": [],
            "emails": ["github-actions[bot]"],
            "allow_patterns": [],
        }
        text = "The force-push therefore ran as `github-actions[bot]` and failed"
        assert contains_agent_fingerprint(text, blocklist) is None

    def test_contains_fingerprint_blocks_email_outside_code_span(self):
        """A bare identity mention on the same line is still matched (Refs: #1521)."""
        blocklist = {
            "trailers": [],
            "names": [],
            "emails": ["github-actions[bot]"],
        ...
        text = "Pushed by github-actions[bot] via `promote-release.yml`"
        assert contains_agent_fingerprint(text, blocklist) == "github-actions[bot]"

    def test_contains_fingerprint_code_span_strip_stays_on_one_line(self):
        """A stray backtick must not swallow a later attribution line."""
        blocklist = {
            "trailers": [],
            "names": [],
            "emails": ["noreply@cursor.com"],
        ...
        text = "See the `promote job\n\nAuthor: Agent <noreply@cursor.com>\n`end"
        assert contains_agent_fingerprint(text, blocklist) == "noreply@cursor.com"

    def test_contains_fingerprint_blocks_signed_off_by_email(self):
        """A Signed-off-by attribution carrying a blocked email still fails."""
        blocklist = {
            "trailers": [],
            "names": [],
            "emails": ["noreply@cursor.com"],
        ...
        content = "fix: adjust parser\n\nSigned-off-by: Agent <noreply@cursor.com>"
        assert contains_agent_fingerprint(content, blocklist) == "noreply@cursor.com"

    def test_contains_fingerprint_blocks_author_line_email(self):
        """An Author: line carrying a blocked identity still fails."""
        blocklist = {
            "trailers": [],
            "names": [],
            "emails": ["github-actions[bot]"],
        ...
        assert contains_agent_fingerprint(content, blocklist) == "github-actions[bot]"

    def test_contains_fingerprint_email_match_stays_case_insensitive(self):
        """Email matching keeps ignoring case (Refs: #1521)."""
        blocklist = {
            "trailers": [],
            "names": [],
            "emails": ["github-actions[bot]"],
        ...
        content = "Author: GitHub-Actions[Bot] <ci@example.com>"
        assert contains_agent_fingerprint(content, blocklist) == "github-actions[bot]"

    def test_contains_fingerprint_code_span_does_not_neuter_trailer_rule(self):
        """Backticking the email inside a trailer does not evade the trailer rule."""
        import re
        ...
        content = "feat: add thing\n\nCo-authored-by: Agent <`noreply@cursor.com`>"
        assert (
            contains_agent_fingerprint(content, blocklist)
            == "Co-authored-by: Agent <`noreply@cursor.com`>"
        )

    def test_contains_fingerprint_code_span_strip_does_not_affect_names(self):
        """Name matching is untouched: an attribution line still blocks (Refs: #1521)."""
        blocklist = {
            "trailers": [],
            "names": ["cursor"],
            "emails": [],
        ...
        assert (
            contains_agent_fingerprint("Generated by `Cursor`", blocklist) == "cursor"
        )
