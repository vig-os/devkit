...
3. **Review and test the release**
   ...
   - Mark the PR as ready for review (required only for the final release,
     once the candidate is verified; no approval is collected yet)
...
4. **Finalize and publish** (validates, finalizes CHANGELOG, builds, tests, signs, and publishes)
   ...
   The workflow will:
   - Validate CI status and that the PR is out of draft
   - Set the release date in CHANGELOG
   - Build and test multi-arch images (amd64, arm64)
   - Scan for vulnerabilities with Trivy
   - Push to GHCR with `:X.Y.Z` tags (`:latest` moves at promote)
   - Sign images with Sigstore cosign
   - Generate SBOM and attest provenance
   - Automatically roll back on failure

5. **Approve and promote** — approve the release PR (the cycle's single human
   approval, after finalize so it covers the release content exactly as it
   ships), then:
   ...
   This publishes the draft GitHub Release, moves `:latest`, and merges the
   release PR into `main`; the `sync-main-to-dev` workflow then automatically
   opens a PR to merge `main` into `dev`. To reject the release instead, run
   `just abandon-release X.Y.Z` (safe only while the Release is a draft).
